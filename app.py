...
BUNDLED_MEDIA_ARCHIVE = BASE_DIR / "source_media.zip"
...
async def add_security_headers(request: Request, call_next):
    ...
    if request.url.path in {"/static/app.js", "/static/style.css", "/service-worker.js"}:
...
def init_db():
    ...
    # Backfill screenshots from a bundled, ticket-specific evidence manifest.
    # This is more reliable in production than extracting PDF image objects at
    # runtime and avoids attaching every image on a shared page to every ticket.
    if BUNDLED_MEDIA_ARCHIVE.exists():
        import_bundled_source_media(conn)
...
def import_bundled_source_media(conn: sqlite3.Connection) -> None:
    """Attach ticket-specific screenshots extracted from the original PDF.

    The deployment ships one compact source_media.zip archive plus a manifest
    keyed by the actual ticket number. This makes GitHub updates easy and is
    reliable on Railway even when optional PDF image decoders are unavailable.
    """
    if not BUNDLED_MEDIA_ARCHIVE.exists():
        return
    try:
        with zipfile.ZipFile(BUNDLED_MEDIA_ARCHIVE, "r") as bundle:
            manifest = json.loads(bundle.read("manifest.json").decode("utf-8"))
            names = set(bundle.namelist())
            # Remove only evidence rows generated by the older page-wide importer.
            # User-uploaded attachments are never touched.
            old_rows = conn.execute(
                "SELECT id FROM attachments WHERE filename LIKE 'Source screenshot · page %' AND stored_name LIKE 'source_p%'"
            ).fetchall()
            for row in old_rows:
                conn.execute("DELETE FROM attachments WHERE id=?", (row["id"],))

            imported_counts: dict[int, int] = {}
            for ticket_no, items in manifest.items():
                ticket = conn.execute("SELECT id, submitted_by FROM tickets WHERE ticket_no=?", (ticket_no,)).fetchone()
                if not ticket:
                    continue
                for item in items:
                    archive_name = Path(item.get("file", "")).name
                    ...
                    safe_no = ticket_no.replace("/", "-")
                    stored = f"doc_{safe_no}_{digest}.webp"
                    ...
                    display_name = f"Original ticket screenshot · page {page_num} · {image_index}.webp"
                    conn.execute(
                        "INSERT INTO attachments(ticket_id,filename,stored_name,content_type,created_by,created_at) VALUES(?,?,?,?,?,?)",
                        (ticket["id"], display_name, stored, "image/webp", ticket["submitted_by"], now()),
                    )
                    imported_counts[ticket["id"]] = imported_counts.get(ticket["id"], 0) + 1
    except Exception:
        return
    ...
    for ticket_id, count in imported_counts.items():
        conn.execute(
            ...
            (ticket_id, None, "Imported source evidence", f"Added {count} original screenshot(s) from the source document.", now()),
        )
    conn.commit()
